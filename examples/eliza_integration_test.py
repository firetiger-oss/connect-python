...
from eliza import ElizaServiceClient
...
async def test_introduce(client: ElizaServiceClient, protocol_name: str) -> bool:
    ...
    try:
        ...
        if responses:
            ...
            print(f"    ERROR: No introduction sentences received")
            return False
    except Exception as e:
        print(f"    ERROR: {e}")
        raise
...
async def test_converse(client: ElizaServiceClient, protocol_name: str) -> bool:
    ...
    try:
        ...
        if responses:
            ...
            print(f"    ERROR: No conversation responses received")
            return False
    except Exception as e:
        print(f"    ERROR: {e}")
        raise
...
async def main():
    ...
    for protocol, name in protocols_to_test:
        try:
            ...
            import traceback
            traceback.print_exc()
            all_results[name] = [False, False, False]
    
    # Summary
    print("\n📊 Test Results Summary")
    print("=" * 60)
    
    method_names = ["Say", "Introduce", "Converse"]
    overall_success = True
    
    for protocol_name, results in all_results.items():
        print(f"\n{protocol_name}:")
        for i, (method, success) in enumerate(zip(method_names, results)):
