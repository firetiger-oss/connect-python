...
class ElizaServiceClient:
    ...
    def converse(self, reqs: StreamInput[ConverseRequest]) -> AsyncIterator[ConverseResponse]:
    ...
    async def converse_stream(
        ...
        return await self._connect_client.call_bidirectional_streaming(url, reqs, ConverseResponse)

    def introduce(self, req: IntroduceRequest) -> AsyncIterator[IntroduceResponse]:
        return self._introduce_impl(req)

    async def _introduce_impl(self, req: IntroduceRequest) -> AsyncIterator[IntroduceResponse]:
        async with await self.introduce_stream(req) as stream:
            async for response in stream:
                yield response

    async def introduce_stream(self, req: IntroduceRequest) -> StreamOutput[IntroduceResponse]:
        url = self.base_url + "/connectrpc.eliza.v1.ElizaService/Introduce"
        return await self._connect_client.call_server_streaming(url, req, IntroduceResponse)
